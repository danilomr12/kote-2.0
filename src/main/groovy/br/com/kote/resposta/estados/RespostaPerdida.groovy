...
class RespostaPerdida extends EstadoResposta {
    ...
    IResposta ressuscite(IResposta resposta) {
        Compra compra = compraService
                .getCompraByCotacaoId(resposta.cotacao.id)
        if (compra.estado == EstadoCotacao.EM_ANALISE
            ...
            compraService.updateEstadoCompraByCotacaoId(
                    resposta.cotacao.id, EstadoCotacao.AGUARDANDO_RESPOSTAS
            )
